...
# NOTE: The following Pydantic models are depth-limited versions of the
# recursive ReportSection model. They are used exclusively for generating a
# non-circular JSON schema for LLM API calls that do not support circular
# references, such as some local LLM servers. This avoids the
# "Converting circular structure to JSON" error.

class ReportSectionBase(BaseModel):
    """Base model for a report section, excluding recursive subsections."""
    model_config: ClassVar[ConfigDict] = ConfigDict(extra='forbid')
    section_id: str = Field(..., description="Unique identifier for the section (e.g., 'introduction', 'lit_review').")
    title: str = Field(..., description="Human-readable title for the report section.")
    description: str = Field(..., description="Brief description of what this section should cover.")
    associated_note_ids: Optional[List[str]] = Field(default=None, description="List of note IDs from the exploratory phase relevant to this section.")
    research_strategy: ResearchStrategy = Field(
        default="research_based",
        description="Defines how the research/writing for this section should be approached."
    )

class ReportSectionL2(ReportSectionBase):
    """Represents the deepest level of nesting (sub-subsections). No further nesting allowed."""
    pass

class ReportSectionL1(ReportSectionBase):
    """Represents a top-level section that can have non-recursive subsections."""
    subsections: List[ReportSectionL2] = Field(default_factory=list, description="List of subsections nested under this section.")

class SimplifiedPlanResponseForSchema(BaseModel):
    """
    A version of SimplifiedPlanResponse that uses a non-recursive outline structure
    for generating a compatible JSON schema for LLMs.
    """
    model_config: ClassVar[ConfigDict] = ConfigDict(extra='forbid', json_schema_extra={
        "required": [
            "mission_goal",
    ...
    report_outline: List[ReportSectionL1] = Field(..., description="The planned structure of the final report, with limited depth.")
